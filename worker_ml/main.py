...
def _training_fase(barcodes: list[str]) -> None:
    ...
    for i, barcode in enumerate(barcodes, 1):
        try:
            # El tercer elemento (rmse previo) no es relevante en la fase de
            # entrenamiento; se ignora explícitamente con _.
            model, is_cold_start, _ = load_or_create_model(barcode)
            ...
            rmse  = calcule_rmse(model, X, y)
            save_model(barcode, model, rmse, new_examples=len(X))

            mode = "Cold Start" if is_cold_start else "Incremental"
            logger.info(
                f"  [{i:>3}/{total}] ✅ {mode:<12} | "
                f"barcode={barcode} | n={len(X):>4} | RMSE={rmse:.3f}"
            )
...
def _execute_worker() -> None:
    ...
    if not barcodes:
        logger.warning("⚠️  Sin productos en la BD. Recomendación: Para pruebas usa data_forge.py")
        sys.exit(0)
